...
const Sidebar = ({ currentView, setView, onLogout }) => {
    ...
    return (
        <div className="hidden md:flex flex-col bg-[#050510] border-r border-slate-800 h-full p-4 transition-all duration-300 w-20 hover:w-64 group z-30 fixed left-0 top-0 bottom-0 shadow-2xl">
            ...
            <div className="flex-1 space-y-1 overflow-y-auto overflow-x-hidden custom-scrollbar">
                {menuItems.map(item => (
                    <button
                        key={item.id}
